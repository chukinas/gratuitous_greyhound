defmodule Dreadnought.Util.Precision do
  ...
  def set_precision(struct) when is_struct(struct) do
    struct |> Map.from_struct() |> set_precision()
  end

  def set_precision(%{} = map) do
    map
    |> Enum.map(&set_precision/1)
    |> Map.new()
  end

  def set_precision({key, value}) do
    {key, set_precision(value)}
  end

  def set_precision(value) when is_float(value) do
    Float.round(value, 1)
  end

  def set_precision(value) when is_integer(value) do
    value * 1.0
  end
end
